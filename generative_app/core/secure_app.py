...
if __name__ == "__main__":
    over_theme = {
        "txc_inactive": "#FFFFFF",
        "menu_background": "#26272f",
        "txc_active": "white",
        "option_active": "#3e404f",
    }
    ...
    def mylogout_cb():
        if user_access_level:
            print(
                "Remove user session of user {} with access level {}".format(
                    username, user_access_level
                )
            )
    ...
    # if we want to auto login a guest but still have a secure app, we can assign a guest
    # account and go straight in
    # app.enable_guest_access()

    # If the menu is cluttered, just rearrange it into sections!
    # completely optional, but if you have too many entries, you can make it nicer by
    # using accordian menus
    path_to_script = ""
    if user_access_level == 1:
        complex_nav = {"About": ["About"]}
        ...
        path_to_script = os.path.join(
            os.getcwd(), "generative_app", "sandboxes", f"{sandboxe_name}.py"
        )
        spec = importlib.util.spec_from_file_location(
            f"{username}_{user_access_level}", path_to_script
        )
        ...
        app.add_app(
            "AppifyAi",
            icon="💬",
            app=ChatBotApp(title="AppifyAi", generative_app_path=path_to_script),
        )
        ...
        complex_nav = {"User Guide": ["User Guide"]}
